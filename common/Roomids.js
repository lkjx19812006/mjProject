...
class Roomids {
  constructor(props) {
    this.config = conf.getConfig('roomids');
  ...
  static instance() {
    if (Roomids.g_Instance == null) {
      Roomids.g_Instance = new Roomids()
    }
    return Roomids.g_Instance
  }
  ...
  createIDsFile(from, to, path, countPath) {
    ...
    console.log('Roomids文件顺序打乱完毕')
    try {
      //写入到文件
      fs.writeFileSync(path, buffer, { flag: 'w' });
      fs.writeFileSync(countPath, 0, { flag: 'w' });
      console.log('Roomids文件写入完成')
    } catch (error) {
      console.log('Roomids文件写入文件出错')
    }
